...
def process_video(input_filename, output_filename):
    ...
    for i in range(total_frames):
        ...
        progress = i / (total_frames - 1) if total_frames > 1 else 0  # [cite: 34]

        alpha = 0.8 + (1.5 - 0.8) * progress  # [cite: 35]

        mean = 128  # Approximation [cite: 36]
        adjusted_frame = np.clip(alpha * (gray_frame - mean) + mean, 0, 255).astype(np.uint8)  # [cite: 36, 37]
...
if __name__ == "__main__":
    ...
    output_filename = "Static_Vignette_Effect_exer5.mp4"  
